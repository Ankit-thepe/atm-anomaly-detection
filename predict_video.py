# Final, corrected predict_video.py script
...
ACTION_MODEL_PATH = 'slowfast_best_model.pth' # Your trained model
CLASS_MAP = {
...
def analyze_video(video_path: str):
    ...
    # =============================================================================
    # CORRECTED: Load the base model and manually replace the final layer
    # =============================================================================
    # Step 1: Load the base model architecture without the final layer modification
    action_model = torch.hub.load(
        'facebookresearch/pytorchvideo',
        'slowfast_r50',
        pretrained=False # Important: False since we are loading our own weights
    )

    # Step 2: Replace the final classification layer (the "head")
    # Get the number of input features for the classifier
    num_ftrs = action_model.blocks[6].proj.in_features
    # Create a new layer with the correct number of output classes
    action_model.blocks[6].proj = torch.nn.Linear(num_ftrs, NUM_CLASSES)
    # =============================================================================
    
    # Now, load your fine-tuned weights. This will work because the architectures match.
    ...
    while cap.isOpened():
        ...
        if len(clip_frames_buffer) == CLIP_DURATION:
            ...
            slow_pathway = torch.index_select(
                video_tensor, 2,
                torch.linspace(0, CLIP_DURATION - 1, CLIP_DURATION // ALPHA).long().to(DEVICE)
            )
        ...
        cv2.imshow('ATM Action Recognition', frame)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Analyze a video for custom ATM actions using a fine-tuned SlowFast model.")
